<template>
  <div
    ...
    @click="onEditorClick"
  >
    <div id="dashboard-grid-overlay" class="can-cancel-chart-target"></div>
  </div>
</template>
...
export default {
  ...
  computed: {
    backgroundStyle() {
      ...
      return {
        ...
        background: /^#([0-9a-fA-F]{6}|[0-9a-fA-F]{3})$/.test(root.background)
          ? root.background
          : `url(${root.background}) no-repeat 100% 100%`
      };
    },
    nodes() {
  ...
  methods: {
    ...
    ondrop(event) {
      ...
      if (!eventData || !eventData.type || eventData.type == "") return;

      this.$store.commit("setSelectedId", eventData.eleId);
      var hasNode = this.$store.getters.hasNodeById(eventData.eleId);
      if (hasNode) return;

      var fileName = eventData.type.split("/")[
        eventData.type.split("/").length - 1
      ];
      ...
      if(fileName in this.$config){
        var atts = this.$config[fileName];
        Object.keys(atts).forEach(key => {
          if (key.indexOf("style.") != -1) {
            if (!newNode.style) {
              newNode.style = {};
            }
            newNode.style[key.split(".")[1]] = atts[key].defaultValue;
          }
      ...
      Promise.all([
        import(`../user_components/${eventData.type}.vue`),
      ]).then(([chlid]) => {
        self.createComponent.call(self, eventData.eleId, position, chlid.default);
      });
